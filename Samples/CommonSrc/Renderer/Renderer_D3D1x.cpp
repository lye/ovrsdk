...
Content     :   Renderer implementation  for D3DX10/11.
...
static const char* DirectVertexShaderSrc =
    ...
    "void main(in float4 Position : POSITION, in float4 Color : COLOR0, in float2 TexCoord : TEXCOORD0, in float3 Normal : NORMAL,\n"
    "          out float4 oPosition : SV_Position, out float4 oColor : COLOR, out float2 oTexCoord : TEXCOORD0, out float3 oNormal : NORMAL)\n"
    "{\n"
...
static const char* TexturePixelShaderSrc =
    ...
    "float4 main(in Varyings ov) : SV_Target\n"
    "{\n"
    "   return ov.Color * Texture.Sample(Linear, ov.TexCoord);\n"
    "}\n";
...
Renderer::Renderer(const RendererParams& p, HWND window)
{
    RECT rc;
    ...
    Device->CreateInputLayout(ModelVertexDesc, 4, vsData->GetBufferPointer(),
                              vsData->GetBufferSize(), &ModelVertexIL.GetRawRef());
   ...
   // rs.CullMode = D3D1x_(CULL_NONE);
    ...
    const Render::Vertex QuadVertices[] =
        {Vertex(Vector3f(0,1,0)), Vertex(Vector3f(1,1,0)), Vertex(Vector3f(0,0,0)), Vertex(Vector3f(1,0,0))};
...
{
    Renderer* renderer = (Renderer*)dwData;
...
void Renderer::UpdateMonitorOutputs()
{
    HRESULT hr;
...
bool Renderer::SetParams(const RendererParams& newParams)
{
...
bool Renderer::SetFullscreen(DisplayMode fullscreen)
{
...
void Renderer::SetMultipleViewports(int n, const Viewport* vps)
{
    if (n > 2)
        n = 2;
    for (int i = 0; i < n; i++)
...
static int GetDepthStateIndex(bool enable, bool write, Renderer::CompareFunc func)
{
    if (!enable)
        return 0;
...
Texture* Renderer::GetDepthBuffer(int w, int h, int ms)
{
...
ID3D10Blob* Renderer::CompileShader(const char* profile, const char* src, const char* mainName)
{
    ID3D10Blob* shader;
    ID3D10Blob* errors;
...
ShaderBase::ShaderBase(Renderer* r, ShaderStage stage)
...
void Renderer::SetCommonUniformBuffer(int i, Render::Buffer* buffer)
{
...
Render::Shader *Renderer::LoadBuiltinShader(ShaderStage stage, int shader)
{
    switch (stage)
    {
    case Shader_Vertex: return VertexShaders[shader];
    case Shader_Pixel: return PixelShaders[shader];
    default:
        return NULL;
    }
}

ShaderBase* Renderer::CreateStereoShader(PrimitiveType prim, Render::Shader* vs)
{
...
Fill* Renderer::CreateSimpleFill(int flags)
{
    OVR_UNUSED(flags);
    return DefaultFill;
}
...
ID3D1xSamplerState* Renderer::GetSamplerState(int sm)
{
    if (SamplerStates[sm])
...
Texture::Texture(Renderer* ren, int fmt, int w, int h) : Ren(ren), Tex(NULL), TexSv(NULL), TexRtv(NULL), TexDsv(NULL), Width(w), Height(h)
{
    OVR_UNUSED(fmt);
...
void Renderer::SetTexture(Render::ShaderStage stage, int slot, const Texture* t)
{
...
Texture* Renderer::CreateTexture(int format, int width, int height, const void* data)
{
    ...
    {
    case Texture_RGBA:  bpp = 4; d3dformat = DXGI_FORMAT_R8G8B8A8_UNORM; break;
    case Texture_R:     bpp = 1; d3dformat = DXGI_FORMAT_R8_UNORM; break;
    case Texture_Depth: bpp = 0; d3dformat = DXGI_FORMAT_D32_FLOAT; break;
    default:
        return NULL;
    }
...
void Renderer::SetRenderTarget(Render::Texture* color, Render::Texture* depth, Render::Texture* stencil)
{
    OVR_UNUSED(stencil);
...
void Renderer::Render(const Fill* fill, Render::Buffer* vertices, Render::Buffer* indices,
